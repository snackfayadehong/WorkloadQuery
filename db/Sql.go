...
select a.MEnName ,COUNT( b.SpecModelName )as prodSpecNum ,count(distinct billno) as billNum,CONVERT(varchar,CONVERT(decimal(18,2),sum(b.qty * b.BuyPrice))) as totalAmount from T_Prod_Enter a
...
count( distinct a.DepartmentCollarID) as  DpBillNum, CONVERT(decimal(18,2),sum(b.Amount*b.RealUnitPrice))as DpTotalAmount from TB_DepartmentCollar a
